	...
	private void loadData()
	{
		WebServiceHelper mCarInfoServiceHelper = new WebServiceHelper(
			new WebServiceHelper.RequestCallback<Define.QUESTION_CORRELATION>() 
			{
				...
				{
					AlertHelper.getInstance(CorrelationActivity.this).hideLoading();
					if(result == null){
						AlertHelper.getInstance(CorrelationActivity.this).showCenterToast(R.string.info_load_fail);
					...
					else
					{
						if(result == null && data.size()>0){
							AlertHelper.getInstance(CorrelationActivity.this).showCenterToast(R.string.load_done);
						}
						...
						for(int i=0,j=result.data.size(); i<j; i++){
							...
							map.put("attacpathname", result.data.get(i).attacpathname);
							map.put("terminalid", result.data.get(i).terminalid);
							map.put("attacpath", result.data.get(i).attacpath);
