...
public class CalendarView extends View implements View.OnTouchListener {  
    ...
    private List<Integer> checkedIndex = new ArrayList<Integer>();
    private List<Boolean> checkedIndexType = new ArrayList<Boolean>();//选中日期是否是上个月或者下个月的
...
//        surface.width = getResources().getDisplayMetrics().widthPixels;  
    	surface.width = ScreenHelper.dip2px(getContext(), 283);
//        surface.height = (int) (getResources().getDisplayMetrics().heightPixels*2/5);  
        surface.height = ScreenHelper.dip2px(getContext(), 190);
...
//      }  
    ...
    protected void onLayout(boolean changed, int left, int top, int right,  
            int bottom) {  
//        LogHelper.i(TAG, "[onLayout] changed:"  
...
//      canvas.drawRect(surface.weekHeight, surface.width, surface.weekHeight, surface.width, surface.cellBgPaint);  
        ...
        for (int i = 0; i < 42; i++) {  
            ...
            if (isLastMonth(i)) {  
                color = surface.lastMothColor;  
            } else if (isNextMonth(i)) {  
                color = surface.lastMothColor;  
            }else if(checkedIndex.contains(i) || todayIndex != -1 && i == todayIndex){
            	color = Color.WHITE;
            }
...
//        LogHelper.i(TAG, "day in week:" + dayInWeek);  
        ...
        calendar.set(Calendar.DAY_OF_MONTH, 0);  
        // LogHelper.i(TAG, "m:" + calendar.get(Calendar.MONTH) + " d:" +  
    ...
    private void drawCellBg(Canvas canvas, int index, int color) {  
        ...
        canvas.drawRect(left, top, left + surface.cellWidth, top + surface.cellHeight  
...
//                section[1] = 41;  
//            }
//            LogHelper.i("tag", "section:"+section[0]+"--"+section[1]);
...
//            } 
            ...
            for(int i=0; i<checkedIndex.size(); i++){
            	if(checkedIndexType.get(i)){
            		drawCellBg(canvas, checkedIndex.get(i), surface.lastMonthSelectedColor);
            	}else{
            		drawCellBg(canvas, checkedIndex.get(i), surface.cellSelectedColor);
            	}
            }
//            LogHelper.i("tag", "todayNum:"+todayNum);
            if(todayNum >= 0){
            	drawCellBg(canvas, todayNum, surface.todayNumberColor);
            }
//        }  
    }
...
//        	try {
//				mDates.add(mDateFormat.parse(checkDate.get(i)));
...
//        }
        ...
        checkedIndex.clear();
        checkedIndexType.clear();
        
        int status = -1; //-1月前，0月中，1月后
        Date todayDate = new Date(System.currentTimeMillis());
        String strToday = mDateFormat.format(todayDate);
...
//     			LogHelper.i("tag", "月开始");
        	...
        	if(checkDate.contains(mDateFormat.format(mDate))){
    			checkedIndexType.add(status == -1 || status == 1);
        		checkedIndex.add(i);
//        		LogHelper.i("tag", "已签到："+"time:"+mDateFormat.format(mDate));
        	}
//        	Log.i("tag", "strToday:"+strToday+"--day:"+mDateFormat.format(mDate));
        	if(strToday.equals(mDateFormat.format(mDate))){
        		todayNum = i;
        	}
        	
        	if(status == 0){
        		Calendar tempCarlendar = mCalendar;
                tempCarlendar.set(Calendar.DAY_OF_MONTH, 1);
        		 tempCarlendar.set(Calendar.DAY_OF_MONTH, 1);
                 tempCarlendar.roll(Calendar.DAY_OF_MONTH, -1);
        		if(mDate.getDate() == tempCarlendar.getTime().getDate()){
//        			LogHelper.i("tag", "月结束");
        			status = 1;
        			mCalendar.add(Calendar.MONTH, 1);
        			mDate = mCalendar.getTime();
    ...
    private void findSelectedIndex(int startIndex, int endIndex,  
        ...
        for (int i = startIndex; i < endIndex; i++) {  
            ...
            // LogHelper.i(TAG, "temp:" + temp.toLocaleString());  
...
//      }  
        ...
        if (y > surface.monthHeight + surface.weekHeight) {  
            ...
            downIndex = (n - 1) * 7 + m - 1;  
//            LogHelper.i(TAG, "downIndex:" + downIndex);  
            calendar.setTime(curDate);  
            ...
            SimpleDateFormat mDateFormat = new SimpleDateFormat("yyyy-MM-dd");
//            LogHelper.i(TAG, "downData:"+mDateFormat.format(downDate));
        }  
        invalidate();  
    }  
...
//                // LogHelper.i(TAG, "downdate:" + downDate.toLocaleString());  
//                //LogHelper.i(TAG, "start:" + selectedStartDate.toLocaleString());  
//                //LogHelper.i(TAG, "end:" + selectedEndDate.toLocaleString());  
//                downDate = null;  
//                invalidate();  
//            }  
        	...
        	if(Math.abs(offsetX) > 80){
        		if(offsetX > 0){
        			...
        			if(tempDate.getMonth() != curDate.getMonth() 
        					|| tempDate.getYear() != curDate.getYear()){
        				clickRightMonth();
        			}
        		}
        	}
    ...
    public void setSignUpDay(List<String> days){
    	checkDate = days;
    ...
    private class Surface {  
        ...
        private int lastMothColor = Color.parseColor("#bcbcbc");
        public int todayNumberColor = Color.RED;  
        public int cellDownColor = Color.parseColor("#CCFFFF");  
        public int cellSelectedColor = Color.parseColor("#00c4e9");
        public int lastMonthSelectedColor = Color.parseColor("#f3f3f3");
...
//            weekHeight = (float) ((temp + temp * 0.3f) * 0.7);  
            weekHeight = (height - monthHeight - weekHeight) / 6f;
            cellHeight = (height - monthHeight - weekHeight) / 6f;
//            cellHeight = ScreenHelper.dip2px(getContext(), 25);
            cellWidth = width / 7f;  
            borderPaint = new Paint();  
            borderPaint.setColor(borderColor);  
            borderPaint.setStyle(Paint.Style.STROKE);  
            borderWidth = ScreenHelper.dip2px(getContext(), 2);
...
//            float textSize = cellHeight * 0.4f;  
            float textSize = ScreenHelper.dip2px(getContext(), 15);
//            LogHelper.i(TAG, "text size:" + textSize);  
